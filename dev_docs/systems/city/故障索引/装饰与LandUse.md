...
| ID | 状态 | 现象与根因摘要 | 记录 |
| --- | --- | --- | --- |
| `CITY-DL-20260722-01` | 已修复，待新区块实机复验 | LandUse 曾把 owner 内全部 placement 和 BASE/NBT/CROP/BOUNDARY 绑为同一失败粒度，局部 prefab 内容不适配会取消其他正常落地。现按完整 footprint 冻结跨 owner `MATERIALIZE/FALLBACK` 决议，fallback 精确恢复作物与边界，系统错误仍整 owner 回滚。 | `CityNbtPrefabBatchPlacerTest`、`CityLandUseChunkCompilerTest`、`CityLandUseChunkExecutorTest`、`CityLandUseSurfacePrefabOwnerCompilerTest`、`CityLandUseWorldgenRegistryTest`；[故障修复案](../../../../../designer_territoryMod/docs/systems/city/40_tests/故障修复案/20260722_LandUseOwner事务失败粒度过大.md) |
