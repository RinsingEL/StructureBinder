...
| ID | 状态 | 现象与根因摘要 | 记录 |
| --- | --- | --- | --- |
| `CITY-TW-20260908-01` | 代码与独立 GameTest 通过；完整新城实景待验收 | 同一 origin chunk 已有 StructureStart 就跳过后续建筑，导致计划有但现场缺失。现按建筑身份合并多个 piece，NBT 重载验证保留两个独立实例，模板实际底板下按需补基础。 | [任务记录](../archive/398129d_v0.1.0_逐区设计与地形保护实现/任务记录.md) |
